...
<div id="bookDetail">
    ...
    <div class="cont">
      ...
      <p class="showTit">${userShow.tfShow.sGenre} &lt;${userShow.showDetail.showName}&gt;</p>
      <dl>
        <dt>예매번호</dt>
		<dd>${userOrder.orderCode }</dd>
		<dt>관람일</dt>
		<dd>${fn:substring(userShow.showDetail.showDaytime,0,16)}</dd>
		<dt>매수</dt>
      ...
      <dl>
        ...
        <c:choose>
			<c:when test="${empty userRefund}">
				<dt>예매취소</dt>
        		<dd>
        			<form action="${pageContext.request.contextPath}/cancle_book" method="post">
        				<input type="hidden" name="showNum" value="${userShow.showDetail.showNum}"/>
        				<input type="hidden" name="orderCode" value="${userOrder.orderCode}"/>
        				<button type="submit">클릭하여 예매 취소</button>
        			</form>
        		</dd>
			</c:when>
			
			<c:otherwise>
				<dt>${userRefund.refundState}일</dt>
				<dd>${fn:substring(userRefund.refundDate,0,16)}</dd>
			</c:otherwise>
		</c:choose>
      </dl>
    </div>
