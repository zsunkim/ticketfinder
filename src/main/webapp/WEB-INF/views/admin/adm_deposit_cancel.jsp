...
<body>
				...
				<div id="page-content">
					...
					<div class="panel">
						<div class="panel-heading">
							<h3 class="panel-title">미입금 주문 취소</h3>
						</div>
						<div id="demo-custom-toolbar2" class="table-toolbar-left">
							<button id="demo-dt-delete-btn" class="btn btn-danger btn-labeled fa fa-times">미입금 주문 취소</button>
						</div>
						<div class="panel-body">
							<form name="unpaidOrder" method="post" id="unpaidForm" action="${pageContext.request.contextPath }/depositCancel.admin">
							<table id="demo-dt-delete2" class="table table-striped table-bordered" cellspacing="0" width="100%">
								<thead>
									<tr>
										<th>주문번호</th>
										<th>주문자</th>
								...
								<tbody>
									<c:forEach var="longUnpaind" items="${longUnpaidOrderList }">
									<tr>
										<td>${longUnpaind.tfOrder.orderCode }</td>
										<td>${longUnpaind.tfOrder.orderUserName }</td>
										<td>${longUnpaind.tfOrder.orderUserId }</td>
										<td>${longUnpaind.showDetail.showName }</td>
										<td>${fn:substring(longUnpaind.showDetail.showDaytime, 0, 16) }</td>
										<td>${longUnpaind.tfOrder.orderSeat }</td>
										<td>${longUnpaind.tfOrder.orderQty }</td>
										<td><fmt:formatNumber value="${longUnpaind.tfOrder.orderPrice }" pattern="#,###"/>원</td>
										<td>${longUnpaind.tfOrder.orderDpState }</td>
										<td>${longUnpaind.tfOrder.orderPayment }</td>
										<td>${longUnpaind.tfOrder.orderDate }</td>
									</tr>
									</c:forEach>
								</tbody>
							</table>
	...
	$(document).ready(function() {
		...
		$('#mainnav-menu').children().eq(3).addClass('active-sub').children().eq(1).addClass('in').children().eq(1).addClass('active-link');
		
		var rowDeletion2 = $('#demo-dt-delete2').DataTable({
			"responsive": true,
			"language": {
				"paginate": {
		...
		$('#demo-custom-toolbar2').appendTo($("div.toolbar2"));

		$('#demo-dt-delete2 tbody').on( 'click', 'tr', function () {
			if(!$(this).hasClass('selected')) {
				$(this).addClass('selected');
				var orval=$(this).children().first().text();
				$(this).prepend('<input type="hidden" value="'+orval+'" name="cancelOrderList">');
			} else {
